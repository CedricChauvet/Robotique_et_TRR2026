...
compteur();
ouEstil();
vcible = computeSpeed(FC);
if(FC<dstart){
 needFrein(vcible);
}
else{
 asservVITPID(VIT,vcible);
}
//debug();     
...
void compteur(){
  ...
  if (lastMicro - HallMil > 500) VIT =0;
}
...
float computeSpeed(float dist){
  float vmax=15.0 ;
  float vmin=6.0 ;
  float dstart=150.0 ;
  float dend=50.0 ;
  if(dist>dstart) return vmax;
  if(dist<dend) return vmin;
