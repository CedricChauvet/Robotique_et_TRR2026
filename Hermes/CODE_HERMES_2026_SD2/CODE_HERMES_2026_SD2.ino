...
const char filename[] = "Charbonniere_FCLISS2.csv";
...
if (cumDist>1100){               // arret automatique du hermes
...
// calcul en nominal
else computeSpeed(FCliss);

if(FCliss<dstart){                   // cas dans le virage
 needFrein(vcible);
}
else{
...
void computeSpeed(float dist){
  ...
  float ratio= (FCliss-dend)/(dstart-dend);
...
void debug(){
  ...
  Serial.print( " E ");
  Serial.println(FCliss);                             //affichage distance mesuré par le laser frontal centre,
